const Queries = Object.freeze({
  ...
  AUTH_LOGIN_CREATE: `
      ...
      VALUES ($1, $2, $3) RETURNING id, login, rol`,
  USER_EXISTS: `
  ...
  USER_UPDATE: `
      ...
      SET nombres        = $3,
          ...
          celular        = $7
      WHERE id = $1
        AND numero_documento = $2 RETURNING *
  `,
  CLIENT_CREATE: `
  ...
  CHAT_STATUS_EXISTS: `
      SELECT EXISTS (SELECT 1
                     FROM estado_chat ec
                     WHERE ec.numero_whatsapp = $1) AS exists
  `,
  CHAT_STATUS_CREATE: `
  ...
  APPOINTMENT_CREATE: `
      SELECT *
      FROM fn_insert_appointment($1, $2, $3, $4, $5, $6, $7)
  `,
  APPOINTMENT_UPDATE: `
      SELECT *
      FROM fn_update_appointment($1, $2, $3, $4, $5, $6, $7, $8)
  `,
  ...
  PRODUCT_PROFESSIONAL_EXISTS: `
      SELECT EXISTS (SELECT 1
                     ...
                     WHERE pf.usuario_id = $1
                       AND pp.producto_id = $2) AS exists
  `,
  ...
  `,
  MESSAGE_INSERT: `
      INSERT INTO historico_mensajes (numero, direccion, mensaje, sent_at)
      VALUES ($1, $2, $3, COALESCE($4, NOW())) RETURNING id, numero, direccion, mensaje, sent_at
  `,

  CHATS_LIST_LAST_MESSAGE: `
      SELECT DISTINCT
      ON (numero)
          numero, id, direccion, mensaje, sent_at
      FROM historico_mensajes
      ORDER BY numero, sent_at DESC
          LIMIT $1
      OFFSET $2
  `,

  CHATS_LIST_LAST_MESSAGE_SEARCH: `
      SELECT DISTINCT
      ON (numero)
          numero, id, direccion, mensaje, sent_at
      FROM historico_mensajes
  ...
  MESSAGES_BY_NUMBER: `
      SELECT id, numero, direccion, mensaje, sent_at
      FROM historico_mensajes
      WHERE numero = $1
